...
def epsilon_greedy(Q,
                   state,
                   all_actions, # this should be all allowed actions
    ...
    if eps_type == 'constant':
        ...
        rand_choose = np.random.random() < epsilon
        if rand_choose:
            action = np.random.choice(all_actions, 1)[0]
        else:
        ...
        schduler = ScheduleLinear(schedule_timesteps=anneal_timesteps, 
                                  final_p=epsilon_final, 
                                  initial_p=epsilon_initial)
        action = schduler.value(current_total_steps, 
                                Q, state, all_actions)
        # ADD YOUR CODE SNIPPET BETWEENEX  4.2
        ...
        while episode <= self.episode_max and diff >= self.threshold:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(
                    Q, s_current, list_pos, total_steps, 
                ...
                q_cur = Q[s_current, action]
                q_diff = R + self.gamma * np.nanmax(Q[s_next, :])
                Q[s_current, action] = (1 - self.alpha) * q_cur + self.alpha * q_diff
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
